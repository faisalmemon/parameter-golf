...
class Hyperparameters:
    ...
    max_skip_weights = int(os.environ.get("MAX_SKIP_WEIGHTS", 999))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers, max_skip_weights)
...
def main() -> None:
    ...
    val_tokens = load_validation_tokens(args.val_files, args.train_seq_len).to(device=device)
    ...
    base_model = GPT(
        ...
        max_skip_weights=args.max_skip_weights,
    ...
    _compile_mode = os.environ.get("TORCH_COMPILE_MODEL", "")
    if _compile_mode:
        compiled_model = torch.compile(base_model, mode=_compile_mode)
        log0(f"torch_compile_model:mode={_compile_mode}")
    else:
        compiled_model = base_model
        log0("torch_compile_model:disabled")
    ...
    while True:
        ...
        if should_validate:
            ...
            val_loss, val_bpb = eval_val(
                ...
                label=f"eval_periodic_step{step}",
            )
            log0(
